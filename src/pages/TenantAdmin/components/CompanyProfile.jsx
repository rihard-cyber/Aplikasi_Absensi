  ...
  if (!tenant?.id) {
    return (
      <div className="p-10 text-center glass-panel max-w-2xl mx-auto mt-10">
        <h3 className="text-[var(--danger)] font-bold text-xl mb-2">{t('profile.accessDenied')}</h3>
        <p className="text-gray-400">{t('profile.noCompanyAssociated')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-serif font-bold text-white tracking-wide">{t('profile.title')}</h2>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-panel p-8 border border-white/5 flex flex-col items-center text-center">
            ...
            <div className="mt-6">
              ...
              <p className="text-[10px] text-gray-500 uppercase tracking-widest font-black mt-1">{t('profile.identity')}</p>
            </div>
          </div>

          <div className="glass-panel p-6 border border-white/5 bg-[var(--aurora-3)]/5">
             <h5 className="text-xs font-black text-[var(--aurora-3)] uppercase tracking-[0.2em] mb-3">{t('profile.brandingTips')}</h5>
             <p className="text-[11px] text-gray-400 leading-relaxed">{t('profile.brandingTipsDetail')}</p>
          </div>
          ...
          <div className="glass-panel p-6 border border-[var(--warning)]/20 bg-gradient-to-br from-[var(--warning)]/10 to-transparent relative overflow-hidden group">
             ...
             <p className="text-[10px] text-gray-400 leading-relaxed mb-4">{t('profile.joinCodeNote')}</p>
             <div className="flex items-center justify-between bg-[#0B0C10] border border-[var(--warning)]/30 rounded-xl p-3">
               ...
               <div className="flex gap-2">
                 ...
                 >
                   ...
                   <span className="text-[10px] font-bold uppercase tracking-widest hidden sm:block">{t('profile.generate')}</span>
                 </button>
                 <button 
                   type="button"
                    onClick={() => {
                 ...
                 >
                   ...
                   {isCopied && <span className="text-[10px] font-bold uppercase tracking-widest hidden sm:block">{t('profile.copied')}</span>}
                 </button>
               </div>
             </div>
          </div>
          ...
          <form onSubmit={handleUpdate} className="glass-panel p-8 border border-white/5 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <label className="text-xs text-gray-400 uppercase tracking-widest font-bold block mb-2 ml-1">{t('profile.officialName')}</label>
                <div className="relative">
                  <Building size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" />
                  <input required value={tenant.name} onChange={e => setTenant({...tenant, name: e.target.value})} type="text"   className="w-full bg-[#0B0C10] border border-white/20 rounded-2xl pl-12 pr-5 py-4 text-white outline-none transition-all duration-300 placeholder:text-gray-400 focus:outline-none focus:border-[#00C9FF] focus:ring-2 focus:ring-[#00C9FF]/30 hover:border-white/40" />
                </div>
              </div>

              <div className="md:col-span-2">
                <label className="text-xs text-gray-400 uppercase tracking-widest font-bold block mb-2 ml-1">{t('profile.headquartersAddress')}</label>
                <div className="relative">
                  <MapPin size={16} className="absolute left-4 top-4 text-gray-600" />
                  <textarea value={tenant.address || ''} onChange={e => setTenant({...tenant, address: e.target.value})}  placeholder="Masukkan alamat lengkap..."  className="w-full bg-[#0B0C10] border border-white/20 rounded-2xl pl-12 pr-5 py-4 text-white outline-none transition-all duration-300 h-24 resize-none placeholder:text-gray-400 focus:outline-none focus:border-[#00C9FF] focus:ring-2 focus:ring-[#00C9FF]/30 hover:border-white/40" />
                </div>
              </div>

              <div>
                <label className="text-xs text-gray-400 uppercase tracking-widest font-bold block mb-2 ml-1">{t('profile.phone')}</label>
                <div className="relative">
                  <Phone size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" />
                  <input value={tenant.phone || ''} onChange={e => setTenant({...tenant, phone: e.target.value})} type="tel"  placeholder="021-XXXXXXX"  className="w-full bg-[#0B0C10] border border-white/20 rounded-2xl pl-12 pr-5 py-4 text-white outline-none transition-all duration-300 placeholder:text-gray-400 focus:outline-none focus:border-[#00C9FF] focus:ring-2 focus:ring-[#00C9FF]/30 hover:border-white/40" />
                </div>
              </div>

              <div>
                <label className="text-xs text-gray-400 uppercase tracking-widest font-bold block mb-2 ml-1">{t('profile.website')}</label>
                <div className="relative">
                  <Globe size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-600" />
                  <input value={tenant.website || ''} onChange={e => setTenant({...tenant, website: e.target.value})} type="url"  placeholder="https://..."  className="w-full bg-[#0B0C10] border border-white/20 rounded-2xl pl-12 pr-5 py-4 text-white outline-none transition-all duration-300 placeholder:text-gray-400 focus:outline-none focus:border-[#00C9FF] focus:ring-2 focus:ring-[#00C9FF]/30 hover:border-white/40" />
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 flex items-center justify-between border-t border-white/5 mt-8">
              <div className="flex items-center gap-3">
                {saveStatus === 'saving' && <span className="text-xs text-[var(--aurora-1)] animate-pulse">{t('profile.saving')}</span>}
