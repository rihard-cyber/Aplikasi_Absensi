...
if (!window.absensi_back_handlers) {
  window.absensi_back_handlers = [];
}
...
export function registerBackHandler(handler) {
  window.absensi_back_handlers.push(handler);
  
  // Return cleanup function
  return () => {
    window.absensi_back_handlers = window.absensi_back_handlers.filter(h => h !== handler);
  };
}
...
export function executeBackHandlers() {
  if (window.absensi_back_handlers && window.absensi_back_handlers.length > 0) {
    // Execute from newest to oldest
    for (let i = window.absensi_back_handlers.length - 1; i >= 0; i--) {
      try {
        const handled = window.absensi_back_handlers[i]();
        if (handled) {
          return true;
        }
      } catch (e) {
