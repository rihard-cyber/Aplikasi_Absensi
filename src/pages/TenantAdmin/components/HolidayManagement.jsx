...
const HOLIDAY_TYPES = [
  { value: 'NATIONAL', label: 'Hari Besar Nasional', icon: '🇮🇩' },
  { value: 'RELIGIOUS', label: 'Hari Raya Keagamaan', icon: '🕌' },
  { value: 'COMPANY', label: 'Cuti Bersama Perusahaan', icon: '🏢' },
];
...
const HolidayManagement = () => {
  ...
  const fetchData = async () => {
    ...
    const { data: h } = await supabase.from('company_holidays').select('*').eq('tenant_id', p.tenant_id).order('date', { ascending: false });
    if (h) setHolidays(h);
  };

  const handleSave = async () => {
    if (!form.name || !form.date) { toast('Nama dan tanggal libur wajib diisi', 'error'); return; }
  ...
  return (
    <div className="glass-panel p-8">
      <div className="flex justify-between items-center border-b border-white/10 pb-6 mb-8">
        <div>
          <h2 className="text-2xl font-serif font-bold text-white">Kalender Hari Libur</h2>
          <p className="text-sm text-gray-400 mt-1">Atur hari libur nasional, hari raya, dan cuti bersama perusahaan</p>
        </div>
        <button onClick={() => setShowForm(true)} className="px-4 py-2 rounded-xl bg-gradient-to-r from-[var(--aurora-1)] to-[var(--aurora-3)] text-white text-xs font-bold flex items-center gap-2"><Plus size={16} /> Tambah Libur</button>
      </div>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-6 bg-white/5 rounded-2xl border border-white/10">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Nama Hari Libur</label>
              <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full bg-[#1A1C23] border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none" placeholder="Tahun Baru" />
            </div>
            <div>
              <label className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Tanggal</label>
              <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="w-full bg-[#1A1C23] border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none" />
            </div>
            <div>
              ...
              <select value={form.type} onChange={e => setForm({...form, type: e.target.value})} className="w-full bg-[#1A1C23] border border-white/10 rounded-xl px-4 py-3 text-white outline-none">
                {HOLIDAY_TYPES.map(t => <option key={t.value} value={t.value}>{t.icon} {t.label}</option>)}
              </select>
            </div>
          ...
          <label className="flex items-center gap-2 mt-4 cursor-pointer">
            ...
            <span className="text-xs text-gray-400">Berulang setiap tahun</span>
          </label>
        </motion.div>
      )}
      ...
      {Object.entries(grouped).sort(([a],[b]) => b - a).map(([year, items]) => (
        <div key={year} className="mb-8 last:mb-0">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2"><CalendarDays size={16} className="text-[var(--aurora-3)]" /> {year} ({items.length} hari)</h3>
                ...
                <div key={h.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5 hover:border-white/20 transition-all group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-500">{dayName} • {HOLIDAY_TYPES.find(t => t.value === h.type)?.label || h.type} {h.is_recurring ? '🔁' : ''}</p>
                    </div>
                  </div>
      ...
      {!holidays.length && <p className="text-center text-gray-500 py-8 text-sm">Belum ada hari libur. Tambahkan hari libur untuk kalkulasi absensi yang akurat.</p>}
    </div>
  );
};
