...
import { 
  ...
  Shuffle, Home, Layers, Fingerprint, Wifi, Eye, ArrowRight, ArrowUpCircle, Download
} from 'lucide-react';
...
const LandingPage = () => {
  ...
  const [isInstallable, setIsInstallable] = useState(true); // Default to true so users see the option and iOS instructions
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] text-white font-sans overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="hidden md:flex items-center gap-6 lg:gap-8">
              ...
              {isInstallable && (
                <button onClick={handleInstallApp} className="px-5 py-2.5 rounded-xl bg-blue-500/15 border border-blue-500/30 text-blue-400 text-[10px] font-bold uppercase tracking-wider hover:bg-blue-500/25 transition-all flex items-center gap-1.5 shadow-lg shadow-blue-500/5 hover:scale-105 active:scale-95">
                  <Smartphone size={12} />
                  <span>{i18n("Unduh Aplikasi")}</span>
                </button>
              )}
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-[#0B0C10]/95 backdrop-blur-xl border-b border-white/10">
              <div className="px-4 py-6 space-y-3">
                ...
                <div className="flex flex-col gap-3 pt-4 border-t border-white/10">
                  ...
                  {isInstallable && (
                    <button onClick={() => { setMenuOpen(false); handleInstallApp(); }} className="w-full py-3.5 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-400 text-xs font-bold uppercase flex items-center justify-center gap-2">
                      <Smartphone size={16} />
                      <span>{i18n("Pasang / Unduh Aplikasi")}</span>
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            ...
            >
              ...
              <div className="flex flex-wrap gap-4 mb-10">
                <button onClick={() => setShowModal(true)} className="px-8 py-4.5 rounded-2xl bg-gradient-to-r from-[var(--aurora-1)] to-[var(--aurora-3)] text-white font-extrabold text-xs uppercase tracking-wider flex items-center gap-3 hover:shadow-[0_0_30px_rgba(142,45,226,0.4)] transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-500/20">{i18n("Konsultasi Demo Gratis ")}<ChevronRight size={16} /></button>
                {isInstallable && (
                  <button onClick={handleInstallApp} className="px-8 py-4.5 rounded-2xl bg-blue-500/10 border border-blue-500/30 hover:bg-blue-500/20 text-blue-400 font-extrabold text-xs uppercase tracking-wider flex items-center gap-2.5 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-blue-500/5">
                    <Smartphone size={16} />
                    <span>{i18n("Unduh / Pasang Aplikasi")}</span>
                  </button>
                )}
                <button onClick={() => scrollTo('fitur')} className="px-8 py-4.5 rounded-2xl border border-white/10 hover:border-white/30 text-white font-bold text-xs uppercase tracking-wider bg-white/[0.02] hover:bg-white/5 transition-all hover:scale-105 active:scale-95">{i18n("Eksplorasi Fitur ")}</button>
              </div>
      ...
      {/* Install App Modal */}
      <AnimatePresence>
        {showInstallModal && <InstallAppModal onClose={() => setShowInstallModal(false)} isIos={isIos} />}
      </AnimatePresence>
...
const InstallAppModal = ({ onClose, isIos }) => {
  return (
    <motion.div 
      initial={{ opacity: 0 }} 
      animate={{ opacity: 1 }} 
    ...
    >
      <motion.div 
        ...
        exit={{ scale: 0.95, opacity: 0, y: 15 }} 
        onClick={e => e.stopPropagation()} 
        className="w-full max-w-md glass-panel p-6 sm:p-8 relative bg-black/70 border border-white/10"
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-white">
          <X size={18} />
        </button>
        ...
        <div className="text-center py-4">
          <div className="w-16 h-16 rounded-2xl bg-blue-500/20 flex items-center justify-center mx-auto mb-4 border border-blue-500/30 text-blue-400">
            <Smartphone size={32} />
          </div>
          <h3 className="text-lg font-serif font-bold text-white mb-2">{i18n("Pasang Aplikasi SI PRESENSI")}</h3>
          <p className="text-xs text-gray-400 mb-6 leading-relaxed">
            {i18n("Dapatkan akses cepat, notifikasi real-time, dan kestabilan sistem langsung dari layar utama Anda.")}
          </p>

          {isIos ? (
            <div className="bg-white/[0.03] border border-dashed border-white/10 rounded-2xl p-4 text-left space-y-3 mb-6">
              <p className="text-xs text-gray-300 font-bold">{i18n("Panduan Instalasi iOS (Safari):")}</p>
              <p className="flex items-start gap-2.5 text-xs text-gray-400">
                ...
                <span>{i18n("Tap tombol bagikan/share (")} <ArrowUpCircle size={13} className="inline text-blue-400" /> {i18n(") di bagian bawah browser Safari.")}</span>
              </p>
              <p className="flex items-start gap-2.5 text-xs text-gray-400">
                ...
                <span>{i18n("Gulir ke bawah lalu pilih ")} <strong className="text-white">"Add to Home Screen"</strong> {i18n(" atau ")} <strong className="text-white">"Tambahkan ke Layar Utama"</strong>.</span>
              </p>
            </div>
          ) : (
            <div className="bg-white/[0.03] border border-dashed border-white/10 rounded-2xl p-4 text-left space-y-3 mb-6">
              <p className="text-xs text-gray-300 font-bold">{i18n("Panduan Instalasi Android / Desktop:")}</p>
              <p className="flex items-start gap-2.5 text-xs text-gray-400">
                <span className="w-4 h-4 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-400 flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">1</span>
                <span>{i18n("Ketuk tombol Pasang Sekarang di bawah untuk memicu konfirmasi PWA browser.")}</span>
              </p>
              <p className="flex items-start gap-2.5 text-xs text-gray-400">
                <span className="w-4 h-4 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-400 flex items-center justify-center text-[9px] font-bold shrink-0 mt-0.5">2</span>
                <span>{i18n("Atau buka menu browser (titik tiga di kanan atas) lalu pilih ")} <strong className="text-white">"Install app"</strong> {i18n(" atau ")} <strong className="text-white">"Pasang aplikasi"</strong>.</span>
              </p>
            </div>
          )}

          <div className="flex gap-3 justify-center">
            <button onClick={onClose} className="px-6 py-3 rounded-xl border border-white/10 text-xs font-semibold text-gray-400 hover:text-white hover:bg-white/5 transition-all">
              {i18n("Batal")}
            </button>
            {!isIos && (
              <button 
                ...
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-[var(--aurora-3)] to-purple-600 text-white text-xs font-bold shadow-lg hover:shadow-xl hover:translate-y-[-1px] transition-all"
              >
                {i18n("Pasang Sekarang")}
              </button>
            )}
            {isIos && (
              <button onClick={onClose} className="px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-xs font-bold shadow-lg">
                {i18n("Saya Mengerti")}
              </button>
            )}
          </div>
