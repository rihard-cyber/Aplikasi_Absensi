...
const ReimbursementRequest = ({ onBack }) => {
  ...
  const statusStyles = { PENDING: 'bg-[var(--warning)]/10 text-[var(--warning)]', APPROVED: 'bg-[var(--success)]/10 text-[var(--success)]', REJECTED: 'bg-[var(--danger)]/10 text-[var(--danger)]', PAID: 'bg-blue-500/10 text-blue-400' };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6 pb-8">
      ...
      <div className="glass-panel p-6">
        ...
        <p className="text-xs text-gray-400 mb-6">Ajukan klaim biaya untuk kesehatan, transportasi, atau kebutuhan lainnya.</p>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Jumlah Klaim</label>
          ...
          <div>
            <label className="block text-[10px] text-gray-500 uppercase tracking-widest mb-1">Bukti (opsional)</label>
      ...
      {history.length > 0 && (
        <div className="glass-panel p-6">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Riwayat Klaim</h3>
          <div className="space-y-3">
            {history.map(r => (
              <div key={r.id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
                ...
                <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest ${statusStyles[r.status]}`}>{r.status}</span>
              </div>
            ))}
          </div>
        </div>
